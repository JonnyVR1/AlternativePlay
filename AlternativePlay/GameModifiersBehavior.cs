...
namespace AlternativePlay
{
    public class GameModifiersBehavior : MonoBehaviour
    {
        ...
        public void BeginGameCoreScene()
        {
            ...
            currentBeatmap = data.difficultyBeatmap;
            ...
            beatmapCallbacksController = gameScenesManager.currentScenesContainer.Resolve<BeatmapCallbacksController>();

            if (IsTransformNecessary() || Configuration.instance.ConfigurationData.TouchNotes)
            {
        ...
        private void Awake()
        {
            ...
            noteDataColorTypeProperty = typeof(NoteData).GetProperty("colorType");
            sliderDataColorTypeProperty = typeof(SliderData).GetProperty("colorType");
            beatmapDataField = typeof(BeatmapCallbacksController).GetField("_beatmapData", BindingFlags.NonPublic | BindingFlags.Instance);
        }
        ...
        private bool IsTransformNecessary()
        {
            ...
            string serializedName = currentBeatmap.parentDifficultyBeatmapSet.beatmapCharacteristic.serializedName;

            if (serializedName == OneSaberModeName && IsOnlyOneColorSelected())
            {
                AlternativePlay.Logger.Info($"No need to transform: {currentBeatmap.level.songName} for One Color as it is already a One Saber map");
                return false;
            }

            if (serializedName == NoArrowsModeName && AreOnlyNoArrowsOptionsSelected())
            {
                AlternativePlay.Logger.Info($"No need to transform: {currentBeatmap.level.songName} to No Arrows as it's already a No Arrows map");
                return false;
            }
        ...
        private IEnumerator TransformMap()
        {
            ...
            var beatmapData = beatmapDataField.GetValue(beatmapCallbacksController) as BeatmapData;
            ...
            TransformNotes(beatmapData, undesiredNoteType);
            TransformSliders(beatmapData, undesiredNoteType);
        ...
        private void TransformNotes(BeatmapData beatmapData, ColorType undesiredNoteType)
        {
            ...
            foreach (NoteData note in beatmapData.GetBeatmapDataItems<NoteData>(0))
            {
                ...
                {
                    SetNoteColor(note, undesiredNoteType.Opposite());
                }
        ...
        private void TransformSliders(BeatmapData beatmapData, ColorType undesiredNoteType)
        {
            ...
            if (config.NoSliders)
            {
                ...
                var burstSliders = beatmapData.GetBeatmapDataItems<SliderData>((int)SliderData.Type.Normal).Where(s => s.sliderType == SliderData.Type.Burst).ToList();
                burstSliders.ForEach(s => beatmapData.allBeatmapDataItems.Remove(s));
            }

            foreach (SliderData slider in beatmapData.GetBeatmapDataItems<SliderData>((int)SliderData.Type.Normal))
            {
                // Transform for One Color if this is the other note type
                if (config.OneColor && slider.colorType == undesiredNoteType)
                {
                    SetSliderColor(slider, undesiredNoteType.Opposite());
                }
            }
        }
        ...
        {
            noteDataColorTypeProperty.SetValue(noteData, color, BindingFlags.NonPublic | BindingFlags.Instance, null, null, CultureInfo.CurrentCulture);
        }
        ...
        {
            sliderDataColorTypeProperty.SetValue(sliderData, color, BindingFlags.NonPublic | BindingFlags.Instance, null, null, CultureInfo.CurrentCulture);
        }
    }
}
